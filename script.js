...
function performComparisonAndDisplay(lastWeekCsvText, currentWeekCsvText) {
    ...
    lastWeekRows.forEach(row => {
        ...
        const key = matchColumns.map(col => row[col] === undefined || row[col] === null ? '' : String(row[col])).join('__DELIMITER__'); // Use a unique delimiter
    ...
    updatedCurrentWeekData = currentWeekRows.map(currentRow => {
        ...
        const key = matchColumns.map(col => currentRow[col] === undefined || currentRow[col] === null ? '' : String(currentRow[col])).join('__DELIMITER__');
